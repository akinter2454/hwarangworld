# Supabase 학급·콘텐츠 기능 연결 방법 (v0.8, 선택 기능)

> 중요: v0.8의 기본 여행, 콘텐츠 제작, 무료 자동 문항 생성, 로컬 분석에는 Supabase가 필요하지 않습니다. 아래 설정은 **여러 학생 기기의 진행도를 교사 한 화면으로 모을 때만** 사용합니다.
...
v0.8에서는 Gemini/OpenAI API를 호출하지 않는다. 문항 생성과 학급 분석은 브라우저의 로컬 엔진으로 처리한다.
