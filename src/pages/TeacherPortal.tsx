...
function downloadCsv(classroom: Classroom, students: StudentRosterRow[]) {
  const rows = [['여행자', '그룹', '방문국가', '별', '퀴즈평균', '미니게임', '마지막동기화']];
  students.forEach((student) => {
    ...
    rows.push([
      student.nickname, student.group_name || '-', String(player?.visitedCountries.length ?? 0), String(player?.stars ?? 0), `${average}%`, String(player?.activityCompletions.length ?? 0),
...
export function TeacherPortal() {
  ...
  if (!cloudConfigured) return <section className="page teacher-page"><header className="page-header"><div><p className="eyebrow">교사용 · 완전 무료 로컬 모드</p><h2>{portalView === 'studio' ? '🧭 세계여행 콘텐츠 스튜디오' : '🧑‍🏫 세계여행 관제센터'}</h2></div><div className="header-actions"><div className="teacher-local-chip">FREE LOCAL</div><button className="small-button" onClick={()=>setPortalView(portalView === 'studio' ? 'classes' : 'studio')}>{portalView === 'studio' ? '← 관제센터' : '🧭 콘텐츠 제작'}</button></div></header>{portalView === 'studio' ? <><div className="local-free-note">이 모드에서는 만든 여행지가 브라우저에 저장되고 같은 기기의 학생 세계지도에 즉시 추가됩니다. 서버·API 키·사용료가 필요 없습니다.</div><TeacherContentStudio /></> : <><CloudSetupGuide /><TeacherDashboard embedded /></>}</section>;
