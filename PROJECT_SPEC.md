...
- 기본 12개국에 약 10개씩의 표현 세트를 제공한다.
- 표현은 인사·예절·학교·일상으로 분류하며 TTS, 배운 말, 즐겨찾기를 지원한다.
- 하단 `말 도감`에서 모든 여행지 표현을 검색·필터링한다.
- 기본 여행지에는 풍경·음식·생활·문화의 4종 로컬 SVG 이미지 자산을 제공한다.
- 이미지 자료는 문화 전체를 대표한다고 설명하지 않고 탐색·비교 자료로 사용한다.
- 교사 제작 콘텐츠는 추가 표현과 선택형 대표 이미지 파일을 저장할 수 있다.
